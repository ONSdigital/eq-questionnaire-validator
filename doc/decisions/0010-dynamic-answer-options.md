...
- `dynamic_options.transform`
    - An optional property used to apply some transform on each value provided to `dynamic_options.values` which will be used for
...
- The `date` operator takes an offset object; this currently only supports `days`, `months`, and `years`.
    This has been extended to support a `day_of_week` property, allowing a date to be set to a specific day for the given week. This can be combined with the numerical offsets to achieve behaviours such as "last Monday", "next Monday" etc.
...
- `values.list_item_selector` is not strictly needed.
    Dynamic options code could always pass the current list item id, and there is already logic in our value source resolver to use that list item id if the answer id is in a repeating section or a list collector block.
...
To make things more straightforward, the outputs below shows the rough representation when using `self` with `map`; the concept is the same when used within `dynamic_options.transform`.
